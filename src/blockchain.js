    ...
    validateChain() {
      ...
      return new Promise(async (resolve, reject) => {
        self.chain.forEach(async (block, index) => {
          ...
          if (!validBlock) {
            errorLog.push(`Block ${block.hash} not validated`)
          }
          if (block.height > 0) {
            if ((await self.getBlockByHeight(index - 1)).hash !== block.previousBlockHash) {
              errorLog.push(`The previousBlockHash of block ${block, index} is not valid`);
            }
          }  
        });
        resolve(errorLog);
